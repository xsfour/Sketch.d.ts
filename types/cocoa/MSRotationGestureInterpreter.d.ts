/// <reference no-default-lib="true" />

declare namespace cocoa {
  export interface MSRotationGestureInterpreter<T = any> extends cocoa.NSObject {
    convertRotation_toLayer<R = number, P0 = number, P1 = unknown>(_convertRotation: P0, _toLayer: P1): R;
    angleOfPoint_inLayer<R = number, P0 = cocoa.CGPoint, P1 = unknown>(_angleOfPoint: P0, _inLayer: P1): R;
    rotationInDegreesInLayer<R = number, P0 = unknown>(_rotationInDegreesInLayer: P0): R;
    rotationInLayer<R = number, P0 = unknown>(_rotationInLayer: P0): R;
    updateWithLocation<R = void, P0 = cocoa.CGPoint>(_updateWithLocation: P0): R;
    updateWithDragRecognizer<R = void, P0 = unknown>(_updateWithDragRecognizer: P0): R;
    centerPoint<R = cocoa.CGPoint>(): R;
    setCenterPoint<R = void, P0 = cocoa.CGPoint>(_v: P0): R;
  }
  namespace classes {
    export interface MSRotationGestureInterpreter<T = any> extends cocoa.classes.NSObject {
      alloc<R = MSRotationGestureInterpreter>(): R;
      new: <R = MSRotationGestureInterpreter>() => R;
    }
  }
}

declare const MSRotationGestureInterpreter: cocoa.classes.MSRotationGestureInterpreter;
